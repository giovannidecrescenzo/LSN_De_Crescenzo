...
//SOLID SIMULATION
  ...
  string output_solid[5] = {"../Solid_sim/output_epot.dat", "../Solid_sim/output_temp.dat", "../Solid_sim/output_ekin.dat"
  , "../Solid_sim/output_etot.dat", "../Solid_sim/output_press.dat"};
  Input(input, config); //Inizialization
  string out_eq = "../Solid_sim/Eq_temp.dat";
  int eq_time_solid = 2000;
  Equilibration_with_print(out_eq, eq_time_solid, 0.8);
  //int nconf = 1;
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
    ...
    Averages(iblk, output_solid);   //Print results for current block
  }
...
//LIQUID SIMULATION
  ...
  config = "config.fcc";
  string output_liquid[5] = {"../Liquid_sim/output_epot.dat", "../Liquid_sim/output_temp.dat", "../Liquid_sim/output_ekin.dat"
  , "../Liquid_sim/output_etot.dat", "../Liquid_sim/output_press.dat"};
  Input(input, config); //Inizialization
  out_eq = "../Liquid_sim/Eq_temp.dat";
  int eq_time_liquid = 2000;
  Equilibration_with_print(out_eq, eq_time_liquid, 1.1);
  //int nconf = 1;
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
    Reset(iblk);   //Reset block averages
    ...
    Averages(iblk, output_liquid);   //Print results for current block
  }  
...
//GAS SIMULATION
  input = "input_gas.in";
  config = "config.fcc";
  string output_gas[5] = {"../Gas_sim/output_epot.dat", "../Gas_sim/output_temp.dat", "../Gas_sim/output_ekin.dat"
  , "../Gas_sim/output_etot.dat", "../Gas_sim/output_press.dat"};
  Input(input, config); //Inizialization
  out_eq = "../Gas_sim/Eq_temp.dat";
  int eq_time_gas = 10000;
  Equilibration_with_print(out_eq, eq_time_gas, 1.2);
  //int nconf = 1;
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
    Reset(iblk);   //Reset block averages
    ...
    Averages(iblk, output_gas);   //Print results for current block
  }  
...
void Equilibration_with_print(string filename, int time, double exp){
  ofstream out;
  out.open(filename);
  for (int i = 0; i < time; i++){
    Move();
    Measure();
    out << i << "\t" << walker[it] << "\t" << exp << endl;
  }
  out.close();
}
...
//Print initial values for measured properties
  ...
  cout << "Initial kinetic ene2rgy   = " << walker[ik]/(double)npart << endl;
...
// distance i-j in pbc
  ...
  walker[iw] = 16.0 * w / vol + rho * walker[it]; //Pressure
...
void Averages(int iblk, string file_out[5]) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Press, EqTemp;
   const int wd=12;
